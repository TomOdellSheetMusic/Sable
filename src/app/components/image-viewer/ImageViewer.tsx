...
export const ImageViewer = as<'div', ImageViewerProps>(
  ({ className, alt, filename, src, requestClose, info, onPrevious, onNext, ...props }, ref) => {
    ...
    // Android's primary action writes to Downloads; the gallery is a separate destination.
    ...
    // Mobile anchors the menu itself, so closing has to clear both anchors.
    ...
    // Web Share needs the decrypted bytes: `src` is a blob or Tauri asset URL that
    // means nothing to the receiving app.
    ...
    const shareActivation = useMobileTapActivation(isMobile, () => {
      void (async () => {
        try {
          ...
          // Cancelling the share sheet rejects; nothing to report.
        }
      })();
    });
